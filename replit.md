...
- **Content Calendar:** Month/week views with enhanced post cards showing scheduled time (e.g., "3:30 PM"), tone badge (if set), platform icon, and caption preview. Includes character limit validation for scheduled posts. **Unified Schedule Drawer** (enabled by default): Single drawer interface for both creating and editing scheduled posts. Create mode accessible via "Schedule Post" button or day cell clicks. Edit mode opens when clicking existing posts, with prefilled data, update/duplicate/delete actions, and intelligent job rescheduling.
