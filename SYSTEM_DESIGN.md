...
- **Key Features:**
    ...
    - **Batch Mode:** Bulk content generation for up to 20 items simultaneously. Features category-specific prompts (Real Estate, Food, Travel, E-commerce, Fashion, Custom), dynamic table for item management, and three action modes: description_only (saves to Drafts), post_now (immediate publishing), and schedule (scheduled posting). Supports multi-platform selection per item and custom tone configuration.
...
- Feature Flags: `FEATURE_TOKEN_REFRESH`, `METRICS_ENGAGEMENT`, `VITE_METRICS_ENGAGEMENT`, `VITE_UI_MODERN_CHART`, `FEATURE_PER_PLATFORM_ANALYTICS`, `VITE_ANALYTICS_INSTAGRAM`, `VITE_ANALYTICS_PINTEREST`, `VITE_ANALYTICS_SHOPIFY`, `VITE_ANALYTICS_ETSY`, `VITE_ANALYTICS_SQUARESPACE`.
